...
from caffe_lib import layer_param
from caffe_lib import caffe_net
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save-root', type=str,
            help='Where to save the dst model and prototxt')
    parser.add_argument('--net', type=str,
            help='PyTorch model name')
    args = parser.parse_args()

    # resnext-wsl archs
    if args.net == 'resnext-101-32x8d':
        net = hubconf.resnext101_32x8d_wsl(True)
    elif args.net == 'resnext-101-32x16d':
        net = hubconf.resnext101_32x16d_wsl(True)
    elif args.net == 'resnext-101-32x32d':
        net = hubconf.resnext101_32x32d_wsl(True)
    elif args.net == 'resnext-101-32x48d':
        net = hubconf.resnext101_32x48d_wsl(True)
    else:
        raise ValueError('Unknow net, please add it.')
    ...
    trans_net(net, input, args.net)

    proto_path = osp.join(args.save_root, '{}.prototxt'.format(args.net))
    model_path = osp.join(args.save_root, '{}.caffemodel'.format(args.net))

    save_prototxt(proto_path)
    save_caffemodel(model_path)
